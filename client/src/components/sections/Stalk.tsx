...
const ParamTag: React.FC<{ param: AIService["parameters"][0] }> = ({
  param,
}) => {
  const getIcon = (type: string) => {
    switch (type.toLowerCase()) {
      ...
      case "number":
        return <Hash size={14} className="mr-1.5" />;
      case "url":
        return <Link size={14} className="mr-1.5" />;
      default:
        return <Type size={14} className="mr-1.5" />;
    }
  };
  return (
...
const ApiTestingPane: React.FC<{ service: AIService; onClose: () => void }> = ({
  service,
  onClose,
}) => {
            ...
            <form onSubmit={handleTestSubmit}>
              ...
              <div className="flex items-center justify-between mt-6">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex items-center justify-center gap-2 bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-all transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-400"
                >
                  {isLoading ? (
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="flex items-center gap-2 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200"
                >
                  <X size={18} /> Close
                </button>
              </div>
...
const Stalk: React.FC = () => {
  ...
  return (
    <>
      <div className="page">
        <h1>Stalk Services</h1>
        <p>Explore, test, and integrate our suite of AI capabilities.</p>
        ...
        <div className="main">
          <table>
            <thead>
              <tr>
                {["No.", "Feature Name", "Status", "Parameters", "Action"].map(
                  (header) => (
                ...
                <React.Fragment key={tool.id}>
                  <tr>
                    ...
                    <td className="text-center">
                      <button
                        onClick={() => toggleTestButton(tool.id)}
